...
public:
    vector<vector<int>> solve(vector<vector<int>>& heights, int type) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                ...
                if (type == 0 && (i == 0 || j == 0)) { //first row and first col
                    q.push({i, j});
                    vis[i][j] = 1;
                }
                if (type == 1 && (i == n - 1 || j == m - 1)) { //last row and last col 
                    q.push({i, j});
                    vis[i][j] = 1;
                }
            }
    ...
    vector<vector<int>> pacificAtlantic(vector<vector<int>>& heights) {
        ...
        vector<vector<int>> vis1 = solve(heights, 0);  //call the function for type = 0 
        vector<vector<int>> vis2 = solve(heights, 1); //call this for type = 1 
